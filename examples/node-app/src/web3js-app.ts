 ...
 * ################################ WARNING ################################
 */
const PRIVATE_KEY = "0x76cef758ca77d603bd1737b3e784a0aa6b901bc0d68e517d4b2fa4899728e52c";
...
export async function web3jsApp() {
    ...
    const initialSenderBalance = await dai.methods.balanceOf(wallet.address).call();
    ...
    const initialRecipientBalance = await dai.methods.balanceOf(recipientAddress).call();
    console.log(Web3.utils.toBN(initialRecipientBalance).div(decimalsBN).toString());

    const ONE_DAI = Web3.utils.toBN(1).mul(decimalsBN);
    const data: string = await dai.methods.transfer(recipientAddress, ONE_DAI).encodeABI();
    const gas = await dai.methods.transfer(recipientAddress, ONE_DAI).estimateGas({ from: wallet.address });
    ...
    const updatedSenderBalance = await dai.methods.balanceOf(wallet.address).call();
    console.log(Web3.utils.toBN(updatedSenderBalance).div(decimalsBN).toString());
    const updatedRecipientBalance = await dai.methods.balanceOf(recipientAddress).call();
    console.log(Web3.utils.toBN(updatedRecipientBalance).div(decimalsBN).toString());
}